...
for /f "usebackq delims=" %%f in (`powershell -NoProfile -Command "(Get-ChildItem '%ROOT%\backend\_backup\mes_*.db' | Where-Object { $_.Name -notlike '*PRE-RESTORE*' } | Sort-Object LastWriteTime -Descending | Select-Object -First 1).FullName"`) do set "LATEST=%%f"
