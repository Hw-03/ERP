...
def build_dashboard(ws, commits):
    ...
    company_commits = count_commits_in_range(commits, "2026-04-21", "2026-05-15")
    kpis = [
        ("개발 기간 (회사)", "2026-04-21 ~ 05-15 (25일 · 근무일 16일)"),
...
def build_summary(ws, commits):
    ...
    rows = [
        ...
        ("2026-05-09 ~ 05-15", "입출고 v2 + 부서 결재",
         "입출고 v2 4단계 마법사·멱등성·위저드 재디자인\n부서 결재 + 직원 권한 분리\nBOM 805건 입력 진행 + 부모 후보 추출\n재고 이미지 컬럼·데이터 모델 정리(legacy 제거)\nCI 안정화·개발 환경 정비", c9, "진행 중"),
    ]
...
def build_features(ws):
    ...
    features = [
        ...
        ("BOM 세팅 도구 (오프라인 · 848 품목)", "BOM",   True,  "805건 입력 진행 (튜닝 잔여)"),
        ...
        ("입출고 내역 사람 친화 개선",           "입출고", True,  "내 요청 카드·상세 표현·표 전면 개선"),
