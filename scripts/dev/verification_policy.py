...
def _docs_gates(files: Sequence[str], reason: str) -> list[dict[str, Any]]:
    labels = {
        "docs-whitespace": "Markdown whitespace",
        "docs-link-tests": "maintained Markdown link checker tests",
        "docs-links": "maintained Markdown links",
    }
    return [
    ...
    *,
    ...
    if mode in {"full", "frontend", "backend", "docs"}:
        ...
        if mode == "full":
            ...
            if selected_has_docs:
                gates = [*_docs_gates(files, "explicit full mode with docs changes"), *gates]
        elif mode == "frontend":
            gates = _full_frontend_gates(files, "explicit frontend mode")
        elif mode == "backend":
            gates = _full_backend_gates(files, "explicit backend mode")
        else:
            gates = _docs_gates(files, "explicit docs mode")
    elif mode == "auto":
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--list-frontend-unit-tests", action="store_true")
    args = parser.parse_args(argv)
    repo_root = args.repo_root.resolve()
    if args.list_frontend_unit_tests:
        paths = _run_git(repo_root, "ls-files", "--cached", "--others", "--exclude-standard").splitlines()
        print(json.dumps(sorted(path for path in paths if is_frontend_unit_test_path(path))))
        return 0
