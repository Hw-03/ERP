...
def randomize(apply: bool = False) -> None:
    ...
    try:
        ...
        if apply:
            db.commit()
            print(f"\n완료: {updated}개 품목 재고 분배")
            print(f"  품절(0): {zero_count}개")
            print(f"  안전재고 설정: {safety_set}개")
            print(f"  경보 예상 (재고 < 안전재고): ~{safety_alert_preview}개")
        else:
            print(f"\n[DRY-RUN] 대상 {updated}개 품목")
