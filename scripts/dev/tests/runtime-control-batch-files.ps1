...
$FrontendReleaseTool = Join-Path $RepoRoot "scripts\ops\employee_frontend_release.py"
...
Assert-ContentMatch $StatusScript 'health/live' "status must report backend process liveness."
Assert-ContentMatch $StatusScript 'health/ready' "status must report backend workload readiness."
Assert-ContentMatch $StatusScript 'alive=\$alive' "status must label liveness separately."
Assert-ContentMatch $StatusScript 'ready=\$ready' "status must label readiness separately."
...
Assert-ContentMatch $WatchServiceScript 'health/live' "backend monitor must report process liveness."
Assert-ContentMatch $WatchServiceScript 'health/ready' "backend monitor must report workload readiness."
Assert-ContentMatch $WatchServiceScript 'watch-backend.*NOT_ALIVE' "backend monitor must label a failed liveness probe."
Assert-ContentMatch $WatchServiceScript 'watch-backend.*NOT_READY' "backend monitor must not render non-ready as normal."
...
Assert-ContentNotMatch $WatchServiceScript 'Get-NetTCPConnection|State\s+:' "watch-service.ps1 must not render a process status dashboard."
...
Assert-ContentMatch $FrontendReleaseTool '"NEXT_PUBLIC_MES_ENV":\s*"employee"' "employee frontend builds must explicitly select the employee environment."
Assert-ContentMatch $FrontendReleaseTool '(?s)environment\s*=\s*os\.environ\.copy\(\).*?environment\.update\(BUILD_ENV\).*?run_command\(npm' "employee builds must use a separate explicit child environment."
...
Assert-ContentMatch $RuntimeControlScript 'health/ready' "runtime-control.ps1 must verify backend readiness before starting the frontend."
Assert-ContentNotMatch $RuntimeControlScript '(?s)function\s+Invoke-ProfileFrontendStartup.*?health/live' "frontend startup must not treat backend liveness as readiness."
...
Assert-ContentMatch $RuntimeControlScript '(?s)function\s+Invoke-ProfileFrontendStartup.*?health/ready.*?Start-ProfileFrontendSupervisor.*?api/app-session.*?backend_proxy_mismatch.*?Stop-SupervisedService.*?Start-ProfileFrontendSupervisor' "frontend startup orchestration must check backend readiness before start and perform one profile-scoped proxy mismatch restart."
...
Assert-ContentMatch $SyncToEmployeeScript 'health/ready' "employee deployment must validate workload readiness after restart."
Assert-ContentNotMatch $SyncToEmployeeScript 'health/live' "employee deployment must not treat process liveness as workload readiness."
Assert-ContentMatch $SyncToEmployeeScript '(?s)Invoke-FrontendRelease -Command ''prepare''.*?\[stop\].*?Invoke-FrontendRelease -Command ''install''.*?\[migrate\]' "employee deployment must prepare before stopping and install the verified release before DB migration."
Assert-ContentMatch $SyncToEmployeeScript '(?s)if\s*\(-not\s+\$frontendInstall\.Success.*?Restore-EmployeePreMigration.*?exit\s+9' "employee deployment must recover the previous release after a failed pre-migration switch."
Assert-ContentMatch $SyncToEmployeeScript '(?s)function\s+Restore-EmployeePreMigration.*?Invoke-FrontendRelease -Command ''rollback''.*?Restart-EmployeeServices' "pre-migration recovery must restore the whole release before restarting services."
