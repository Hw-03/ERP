...
Assert-ContentMatch $StartBackendScript '(?s)if\s*\(\$RuntimeTaskHost\).*?exit\s+\$hostExit.*?Wait-RuntimeHttp200\s+-Url\s+\$ReadyUrl\s+-Attempts\s+120' "the external backend startup caller must wait for workload readiness."
