...
## 11. Inventory Cockpit Update
- [v] `/inventory` Zero-Modal Master-Detail 작업대로 재구성
- [v] 주요 KPI 3개 추가
- [v] 왼쪽 마스터 패널에 검색, 카테고리 필터, LOW/ZERO 재고 상태 추가
- [v] 테이블 선택 스크롤 및 키보드 탐색 및 오른쪽 패널 상세 패널 구현
- [v] ArrowUp / ArrowDown 키보드 탐색 구현
- [v] 오른쪽 패널에서 재고 수량 조정 / 입고 / 출고 처리 구현
- [v] 처리 완료를 알리는 3초 토스트 표시

## 12. 발주 담당자 UX 개편
- [v] StockFilter: LOW(하드코딩 ≤10) → SAFETY(품목별 min_stock 기준)
- [v] getStatusMeta(): min_stock 파라미터 추가, 뱃지 라벨 "발주필요"로 정확화
- [v] KPI Bar "현재고 1~10 품목" → "안전재고 이하 품목" 문구 수정
- [v] kpis.lowCount: 하드코딩 ≤10 → 품목별 min_stock 기반 집계
- [v] 테이블 컬럼: "현재고 / 안전재고" 형식 (qty / min_stock 단위) + 공급사 컬럼 추가
- [v] 상세 패널: 안전재고 인라인 편집 (연필 아이콘 → 입력 → PUT /api/items/{id})
- [v] 상세 패널: 공급사 정보 표시 추가
