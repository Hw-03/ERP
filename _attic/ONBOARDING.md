...
- **검증 게이트**: 커밋 전 저장소 루트에서 `powershell -ExecutionPolicy Bypass -File .\scripts\dev\verify_local.ps1`를 실행한다. `-Mode auto`는 변경 파일을 docs·frontend·backend로 판별하고, `scripts/`·CI·알 수 없는 경로가 섞이면 전체 검증으로 승격한다. frontend coverage 기준은 `frontend/vitest.config.mts`를 정본으로 확인한다.
