...
def build_summary(ws, commits):
    ...
    c17 = count_commits_in_range(commits, "2026-07-04", "2026-07-10")
    c18 = count_commits_in_range(commits, "2026-07-11", "2026-07-16")
    c19 = count_commits_in_range(commits, "2026-07-17", "2026-07-24")
    c20 = count_commits_in_range(commits, "2026-07-25", "2026-08-07")
    c21 = count_commits_in_range(commits, "2026-08-08", "2026-08-21")
...
def build_features(ws):
    ...
    features = [
        ...
        # ── 7/4~7/10 신규 ─────────────────────────────────────
        ("품목 전환 기능", "입출고", True, "출하·입출고 중 품목을 다른 품목으로 전환 처리 (진행 중)"),
        # ── 7/11~7/16 신규 ────────────────────────────────────
        ("AS·연구용 자재 사용출고", "입출고", True, "AS·연구 목적 자재 사용출고 전용 결재·승인 흐름"),
        # ── 7/17~7/24 신규 ────────────────────────────────────
        ("모바일 조립 체크리스트", "생산", True, "교차 확인 검증 + 항목 관리·박스 간 이동"),
        ("자재 입출고관리대장(F704-02)·연간 생산일지(F705-02) 다운로드", "관리", True, "내보내기 화면에 BOM 다운로드 통합"),
        # ── 7/25~8/7 신규 ─────────────────────────────────────
        ("출하 인보이스·수정이력 관리", "출하", True, "준비완료 조건·모바일 안내·이력검색"),
        ("일일 작업 일보 MES 도입", "관리", True, "기존 엑셀 작성 방식 대체"),
        ("설정 화면 통합", "UI", True, "테마·사이드바 고정을 한 화면에서 관리"),
        # ── 8/8~8/21 신규 ─────────────────────────────────────
        ("MES 단독 운영 전환", "재고", True, "엑셀 병행 기록 중단, MES를 재고 업무 단일 공식 기록으로 전환"),
        ("PF별 생산 가능수량 화면", "생산", True, "완제품별 생산 가능수량 확인 + BOM 화면 개편"),
        ("기준 출하품 자동 선정", "생산", True, "담당자 수동 선택 → 시스템 자동 선정"),
        ("품목 선택 전체 화면", "창고", True, "창고 화면 품목 선택을 전체 화면으로 확대"),
        # ── 예정 ─────────────────────────────────────────
