...
def build_guide_sheet(wb: Workbook) -> None:
    ...
    kv(r, "②", "빨강 칸 3개(품목명/공정코드/현재수량)만 채워도 등록 가능");              r += 1
    kv(r, "③", "노랑 칸(규격/단위/부서/모델)은 알면 채우면 좋음");                        r += 1
    kv(r, "④", "회색 칸은 비워도 됨 (품번/바코드 비우면 자동 부여)");                    r += 2

    h2(r, "2. 공정코드 (드롭다운, 18종)");                                               r += 1
    kv(r, "형식", "{부서 계열 1글자}{단계 1글자}. 부서 6종 × 단계 3종 = 18개");          r += 1
    kv(r, "단계", "R = 원자재 / A = 중간공정(반제품) / F = 공정완료(완제품)");           r += 1
    kv(r, "T_ 튜브",   "TR · TA · TF — 튜브 파트 (원자재 / 중간 / 완료)");               r += 1
    kv(r, "H_ 고압",   "HR · HA · HF — 고압 파트");                                       r += 1
    kv(r, "V_ 진공",   "VR · VA · VF — 진공 파트");                                       r += 1
    kv(r, "N_ 튜닝",   "NR · NA · NF — 튜닝 파트");                                       r += 1
    kv(r, "A_ 조립",   "AR · AA · AF — 조립 파트");                                       r += 1
    kv(r, "P_ 출하",   "PR · PA · PF — 출하 파트 (PA=가방 포장, PF=박스 포장)");          r += 2
