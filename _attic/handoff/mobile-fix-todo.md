...
> 이전 8-5의 헤더 압축 완료 이력은 유지한다. 이번 항목은 실제 모바일 화면에서 확인된 접힘·펼침 본문까지 포함한 후속 가독성 개선이다.

### 9-1. 생산 가능수량 상세 팝업 — 접힘·펼침 공통 가독성 재구성
- [ ] 화면/영역: 모바일 대시보드 → `생산 가능 현황` → `자세히 보기`로 여는 `생산 가능수량` 상세 팝업. 상단 3개 수량 설명, 모델 그룹의 접힌 상태, AF를 펼친 상태와 PF 변형 목록.
- 증상: 563×854 실제 화면에서 상단의 긴 설명 3개와 공유 자재 안내가 목록 진입 전부터 시선을 분산시킨다. 접힌 모델 행은 모델명·PF 칩·3개 수량이 서로 가까이 붙고, 펼친 AF/PF 상태는 긴 이름·코드·수량 열·병목/지정 정보가 표처럼 한꺼번에 보여 빠르게 훑기 어렵다.
- 사용자 불편: 출하 대기·빠른 생산·총생산의 의미와 각 수량을 한눈에 대응시키기 어렵고, 펼친 뒤에도 어떤 PF 변형의 수량인지 즉시 파악하기 어렵다.
- 검증된 사실:
  - `frontend/app/mes/_components/CapacityDetailModal.tsx` 54~96줄이 제목, 3개 수량 설명, 공유 자재 안내를 렌더한다.
  - 같은 파일 197~348줄에는 `sm:hidden` 모바일 그룹/AF 카드가 따로 있으며, 데스크톱 표는 367줄 이후 `sm:block`으로 분리돼 있다.
  - PF 변형은 같은 파일의 `PfVariants`에서 이름·3개 수량·지정 액션·상태 아이콘을 한 행 그리드로 렌더하므로, 펼친 모바일 상태에서 정보 밀도가 특히 높다.
- 확정 정책:
  - 설명은 숨기지 않는다. 모바일에서는 `출하 대기 · PF 재고 즉시 출하`, `빠른 생산 · AF·직계 부품 기준`, `총생산 · 전체 BOM 기준`처럼 항목당 짧은 한 줄 요약으로 바꾼다.
  - 접힌 모델, 펼친 AF, PF 변형 모두 **이름 → 보조 정보 → 동일한 3열 수량** 순서를 지킨다. 수량의 이름·색·순서는 항상 `출하 대기 / 빠른 생산 / 총생산`으로 고정한다.
  - 긴 이름은 모바일 카드 안에서 최대 두 줄까지 읽을 수 있게 하고, 가로 스크롤·잘림으로 의미를 잃는 표 레이아웃은 사용하지 않는다.
- 구현 방향:
  - 상단 설명 3개는 작은 색 점과 굵은 라벨, 짧은 보조 문구를 한 줄 행으로 재구성하고, 공유 자재 안내는 컴팩트한 안내 블록으로 유지한다.
  - 접힌 모델 행은 모델명·종수와 지정 PF를 위쪽 정보 영역에 두고, 3개 수량은 얇은 구분선이 있는 공통 3열 블록으로 분리한다.
  - 펼친 AF 카드는 이름(최대 두 줄), 코드, 공통 3열 수량 블록을 위 순서로 둔다. PF 변형은 모바일 전용 카드 분기를 두어 긴 이름·병목·지정/해제 액션을 수량 블록과 분리한다.
  - 데이터 계산, 그룹/AF 접기·펼치기, PF 지정·해제, 상태 색상과 아이콘은 유지한다. `sm:` 이상 데스크톱 헤더와 표 레이아웃은 변경하지 않는다.
- 수용 기준:
  - 393×852와 360px급 모바일 폭에서 팝업을 열면 상단 설명·첫 모델 그룹·3개 수량의 관계를 스크롤 없이 읽을 수 있다.
  - 모델 접힘, AF 펼침, PF 변형 표시 각각에서 긴 이름·코드·병목·지정 상태가 서로 겹치거나 가로 스크롤되지 않는다.
  - 3개 수량은 모든 상태에서 같은 순서와 색으로 표시되고, 그룹 접기/펼치기·PF 지정/해제·닫기 동작이 기존과 동일하다.
- 브라우저 확인 시나리오: DX3000처럼 지정 PF가 있는 그룹과 ADX4000W처럼 여러 AF가 있는 그룹을 각각 접힘·펼침으로 확인한다. PF 지정/해제 후 요약 수량 반영을 확인하고, 1280px 이상 데스크톱에서 기존 표 배치가 변하지 않았는지 비교한다.
- 우선순위: 중간
