...
- [ ] 화면: 모바일 입출고 Step2 `세부 작업과 부서` → `세부 작업`의 `창고 → 부서` / `부서 → 창고` 선택 카드.
...
- [ ] 화면: 모바일 불량 탭 → 불량 격리 Step2 `품목 선택` 하단 `격리하기 (N건)` 버튼. `바로 폐기`의 동일 단계도 같은 구조.
...
- [ ] 화면: 입출고 → `내 요청` 카드에서 품목이 5건을 초과할 때 나타나는 `외 N건 더보기` / `접기` 버튼.
...
- [ ] 화면: 모바일 대시보드 → `생산 가능 현황` → `자세히 보기`로 여는 `생산 가능수량` 상세 팝업.
