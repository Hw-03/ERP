# CP5 W5 완료 · W6 `IC-18` 진입 TODO

## 현재 정본

- 작업 위치: `C:\ERP\.worktrees\full-code-quality-checkpoint-2`
- branch: `codex/full-code-quality-improvement`
- W5 제품 기준 HEAD: `fb339b5dc29013fc97af5035905431177ec54aea`
- W5 제품 commit: `fb339b5dc29013fc97af5035905431177ec54aea`
- 최종 GitHub CI: run `33606010198`, 6/6 success
- 카드 상태: `IC-03`, `IC-06`, `IC-07`, `IC-08`, `IC-17` 완료. CP5 W5 완료. 다음 단일 범위는 W6 `IC-18`이며 아직 시작하지 않았다.
- 엄격한 잔여 IC: 15개. `IC-04`·`IC-20` required-check 외부 증거가 확보되면 13개다.
...
10. W6 완료 전 W7 `IC-19`를 시작하지 않는다.

## W6 완료 증거

- 정상 backup의 manifest와 artifact hash·size·schema·revision·snapshot이 exact 일치한다.
- WAL-only 후속 write가 기존 SQLite backup을 stale로 만든다.
- missing/corrupt manifest, artifact tamper, wrong head, table/column/index 누락, FK·W5 integrity 위반을 모두 fail-closed한다.
- staged restore 실패 시 기존 target과 마지막 valid manifest가 보존된다.
- PostgreSQL dump→폐기 가능한 임시 DB restore→schema/FK/integrity 검증이 실제 PostgreSQL에서 통과한다.
- retention·NAS·cleanup이 artifact/manifest orphan을 만들지 않는다.
...
- 현재 상태는 **W6 시작 직전**이다. W6 작업·thread·제품 변경은 아직 만들거나 시작하지 않았다.
...
- W6를 닫기 전 W7을 시작하지 않고 CP5 완료 전 CP6를 시작하지 않는다.
