...
`internal_use` 는 사용 부서(AS/연구)를 선택하고 실제 차감 라인을 승인 종류별 최대 3건으로
묶는다. 창고 재고는 창고 승인, 부서 재고의 AR·AA 품목은 AS·연구 승인, 그 밖의 부서 재고와
부서 재입고는 부서 승인을 받는다. 활성 AS·연구 승인자가 없으면 AR·AA 라인도 부서 승인으로
고정된다. 모든 승인·반려 결정이 끝나기 전에는 전량 예약만 유지하고 재고나 거래 이력을 만들지
않으며, 마지막 결정 시 승인 라인만 한 트랜잭션에서 반영한다. 승인·반려가 섞이면 배치는
`partially_completed`, 모두 반려되면 `rejected`가 된다. 최종 반영 전 요청자 취소는 같은 작업
배치 전체의 예약과 승인 요청을 취소한다. 품목 전환 메뉴와 실행 API는 조립·출하 부서만 사용할 수 있다.
품목 전환의 구성 차이는 소스와 대상 품목의 직계 BOM 품목·수량만 비교한다. 직계 자식의
하위 BOM은 다시 전개하지 않으며, 미리보기와 실제 차감·회수 원장도 같은 직계 차액을 사용한다.

### 품목코드 자동 부서 경로

`warehouse_to_dept`, `dept_to_warehouse`, `produce`, `disassemble`, `adjust_in`,
`adjust_out` 여섯 작업은 출발·도착 부서를 사용자가 고르지 않는다. 서버가
각 라인의 현재 `Item.process_type_code`에서 생산 부서를 결정하며, 저장·불러오기·제출·결재
실행 직전에 같은 기준으로 경로를 다시 정규화한다. 따라서 한 배치 안의 BOM 상위와 구성품 또는
여러 낱개 품목은 서로 다른 생산 부서로 함께 반영될 수 있다. 화면에는 라인별 실제 경로를
표시하고, 배치 헤더 부서는 반영 라인이 단일 부서일 때만 표시한다. 품목 선택 화면의 부서·모델·
공정 단계 필터는 그대로 사용할 수 있으며, 경로 결정에는 영향을 주지 않는다.

AS·연구 사용출고, 불량 처리, 창고 수량보정은 이 자동 경로 대상이 아니며 기존 선택·처리 규칙을
유지한다. 이 변경에는 스키마 마이그레이션이나 기존 재고 위치 이동이 없다. 잘못된 위치의
기존 재고는 담당자의 낱개 입출고 정정으로 보류하며 여기서 이관하지 않는다.

불량 처리의 `defect_return`은 단건·일괄 모두 반품 실행 전에 활성 공급업체 한 곳을 필수로
선택한다. 선택 업체 ID와 당시 이름은 `StockRequest`에 저장되고, 실제 실행 직전에 업체 활성
상태를 다시 확인한 뒤 `TransactionLog`에도 완료 시점 이름을 스냅샷으로 남긴다. 취소 거래는
원거래 스냅샷을 복사하므로 이후 업체 이름 변경·숨김과 무관하게 `불량 격리 → 업체`,
`업체 → 불량 격리` 이력이 유지된다. 업체 정보가 없는 레거시 반품은 기존 표시를 유지하며,
불량 반품 업체는 F704-02에 기록하지 않는다.
...
- `warehouse_to_dept`, `dept_to_warehouse`, `internal_use_out` 흐름은 즉시 반영되지 않고 `StockRequest`
  로 변환되어 결재 큐에 들어간다. `internal_use_out`은 실제 차감 라인의 출처와 공정 코드에 따라
  창고·AS·연구·부서 요청으로 최대 3건까지 연결된다.
- 결재자는 승인 종류에 따라 창고 정/부, 부서 정/부, 또는 관리자 화면에서 지정한 활성
  AS·연구 승인자다. `internal_use_out`에서는 각 종류에 맞는 권한으로만 결정한다.
- 일반 비자기승인 요청과 `internal_use_out`의 모든 출고 라인은 원천별로 재고를 예약한다. 창고
  출고는 `Inventory.pending_quantity`, 부서 생산·불량 출고는 해당
  `InventoryLocation.pending_quantity`를 사용하며, 입고 전용 요청은 예약 없이 `SUBMITTED`
  상태로 대기한다.
- 일반 요청의 최종 결재는 같은 트랜잭션에서 요청의 모든 source 예약을 한 번 해제한 뒤 실제
  재고 변경과 `TransactionLog` 기록을 수행한다. `internal_use_out`은 개별 승인으로 실행하지 않고,
  연결 요청이 모두 결정된 마지막 시점에 승인된 source만 원자적으로 실행한다. 반려·취소·승인
  실패도 해당 source 예약을 해제한다.
- 일반 자기승인 요청은 즉시 실행할 수 있다. `internal_use_out` 자기승인은 해당 종류의 결정만
  기록하며, 다른 종류가 남아 있으면 예약 상태로 기다린다.
