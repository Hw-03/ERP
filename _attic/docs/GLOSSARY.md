...
| `INTERNAL_USE` | AS·연구 사용출고 | 창고·AS·연구·부서 결정을 모두 마친 뒤 승인 라인만 사내 사용으로 일괄 차감 |
...
선택하면 실제 차감 라인이 창고 재고=`창고 승인`, 부서 AR·AA=`AS·연구 승인`, 그 밖의 부서
재고·재입고=`부서 승인`으로 분류된다. 모든 연결 요청이 승인 또는 반려된 뒤 승인 라인만
원자적으로 반영하며, 혼합 결과 이력의 반려 라인은 `반려 · 재고 미차감`으로 표시한다. 활성
AS·연구 승인자가 없을 때 AR·AA 라인은 부서 승인으로 고정된다. 최종 반영 전에는 작업 배치
전체를 취소해 모든 예약을 해제할 수 있다.
