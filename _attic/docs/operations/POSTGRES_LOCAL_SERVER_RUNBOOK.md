...
3. 약 30초 대기
4. 서버 시작: `python -m uvicorn app.main:app --host 0.0.0.0 --port 8000 --workers 2`
5. preflight 실행: `python scripts/ops/preflight_30_users.py --url http://localhost:8000`

### 백업 복구
```bash
# 백업 파일로 복구 (PostgreSQL)
docker exec -i <postgres-container> psql -U erp_user erp_db < backup_20260508.sql
...
직원 PC에서 서버에 접속하려면 Windows 방화벽에서 포트 8000을 허용:

```powershell
# 관리자 PowerShell에서
New-NetFirewallRule -DisplayName "MES Server" -Direction Inbound -Protocol TCP -LocalPort 8000 -Action Allow
```
