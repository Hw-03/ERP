...
추천 아키텍처는 한 문장으로 요약하면 이렇습니다. **접속은 HTTPS, 엔진은 native fast-path + ZXing fallback, UX는 항상 manual fallback 보장**입니다. 이 구조가 현재 MES 코드베이스와 가장 잘 맞습니다. 공용 모달 하나만 교체하면 창고/부서 입출고 화면 전체가 동시에 개선되기 때문입니다. fileciteturn12file0L1-L1 fileciteturn13file0L1-L1 fileciteturn14file0L1-L1
...
그래서 최종 권장안은 다음 한 줄입니다. **필드 마찰을 최소화하려면 “HTTPS + `BarcodeDetector`는 fast-path만 + `@zxing/browser` fallback + 수동 입력 UI”를 기본 해법으로 채택하고, 오프라인·엔터프라이즈 안정성이 핵심 요구가 되는 시점에만 네이티브 래퍼를 올리는 것이 가장 합리적입니다.** 이 안은 현재 MES 저장소의 공용 모달 구조와도 잘 맞고, Android에서는 빠르며, iOS에서는 적어도 “완전히 막히는 구조”를 피할 수 있습니다. fileciteturn12file0L1-L1 citeturn20search1turn19search0turn15search0turn16search0
...
또한 비교표의 크기 수치는 공식 npm의 unpacked size를 주로 사용한 **참고값**입니다. 실제 앱 초기 다운로드 크기와 런타임 체감은 동적 import 여부, bundler 설정, WebAssembly 별도 자산, 네이티브 프레임워크 포함 여부에 따라 달라집니다. 따라서 기술 선택의 최종 근거는 표의 크기 숫자 하나가 아니라, **현재 MES 코드 구조와 현장 운영 요구**여야 합니다. citeturn20search1turn17search0turn25search4turn16search0