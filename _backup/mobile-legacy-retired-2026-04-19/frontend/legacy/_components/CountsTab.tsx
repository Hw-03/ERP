...
export function CountsTab() {
  ...
  const filteredItems = useMemo(() => {
    ...
    if (!kw) return [];
    return items
      .filter((i) => `${i.item_name} ${i.item_code} ${i.barcode ?? ""}`.toLowerCase().includes(kw))
      .slice(0, 40);
  }, [items, search]);
  ...
  const submit = async () => {
    if (!selectedItem) { setError("품목을 선택하세요."); return; }
    const qty = Number(counted);
    if (Number.isNaN(qty) || qty < 0) { setError("실사량을 확인하세요."); return; }
    try {
      setSaving(true);
      setError(null);
      setInfo(null);
      ...
      setInfo(`완료: ${row.item_name} · ${diffSign}${formatNumber(row.diff)} 조정됨`);
  ...
  return (
    <div>
      ...
      >
        <div className="border-b px-3 py-3" style={{ borderColor: LEGACY_COLORS.border }}>
          ...
          >
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (selectedItem) { setSelectedItem(null); setCounted(""); }
              }}
            ...
            {search && (
              <button
                onClick={() => { setSearch(""); setSelectedItem(null); setCounted(""); }}
                className="text-[11px]"
          ...
          {selectedItem ? (
            ...
            >
              <div>
                ...
                <div className="text-[11px]" style={{ color: LEGACY_COLORS.muted2 }}>
                  ...
                  <span className="font-mono">
                    {formatNumber(selectedItem.available_quantity ?? selectedItem.quantity)}
                  </span>
              ...
              <button
                onClick={() => { setSelectedItem(null); setSearch(""); setCounted(""); }}
                className="text-[11px]"
                style={{ color: LEGACY_COLORS.muted2 }}
              >✕</button>
            </div>
            ...
            >
              {filteredItems.map((item, i) => (
                <button
                  key={item.item_id}
                  onClick={() => { setSelectedItem(item); setSearch(item.item_name); setCounted(String(Number(item.quantity))); }}
                  className="flex w-full items-center justify-between px-3 py-2 text-left"
                  style={{ borderBottom: i < filteredItems.length - 1 ? `1px solid ${LEGACY_COLORS.border}` : "none" }}
                >
                  <div>
                    <div className="text-sm font-semibold">{item.item_name}</div>
                    <div className="text-[10px]" style={{ color: LEGACY_COLORS.muted2 }}>
                      {item.item_code}{item.legacy_part ? ` · ${item.legacy_part}` : ""}
                    </div>
                  </div>
                  <div className="text-right">
                    ...
                    <div className="text-[10px]" style={{ color: LEGACY_COLORS.muted }}>{item.unit}</div>
                  </div>
                </button>
              ))}
            </div>
        ...
        <div className="px-3 py-3">
          ...
          {previewDiff !== null && (
            <div
              ...
              style={{
                color: previewDiff === 0 ? LEGACY_COLORS.muted2 : previewDiff > 0 ? LEGACY_COLORS.green : LEGACY_COLORS.red,
              }}
            >
              {previewDiff === 0 ? "변동 없음" : `${previewDiff > 0 ? "+" : ""}${formatNumber(previewDiff)} 조정 예정`}
            </div>
          )}

          <div className="mb-3 grid grid-cols-2 gap-2">
            <div>
              <div className="mb-1 text-[10px]" style={{ color: LEGACY_COLORS.muted2 }}>담당자</div>
              <input
                type="text"
                value={operator}
            ...
            <div>
              <div className="mb-1 text-[10px]" style={{ color: LEGACY_COLORS.muted2 }}>사유</div>
              <input
                type="text"
                value={reason}
          ...
          {error && (
            <div className="mb-2 rounded-lg px-3 py-2 text-xs" style={{ background: "rgba(242,95,92,.1)", color: LEGACY_COLORS.red }}>
              {error}
            </div>
          )}
          {info && (
            <div className="mb-2 rounded-lg px-3 py-2 text-xs" style={{ background: "rgba(31,209,122,.1)", color: LEGACY_COLORS.green }}>
              {info}
            </div>
          )}
      ...
      <div className="mb-2 text-xs font-bold" style={{ color: LEGACY_COLORS.muted2 }}>최근 실사 이력</div>
      {counts.length === 0 ? (
        <div className="py-6 text-center text-sm" style={{ color: LEGACY_COLORS.muted2 }}>이력이 없습니다.</div>
      ) : (
        <div
            ...
            <div key={c.count_id} className="flex items-center justify-between gap-3 px-3 py-3">
              ...
              <div className="shrink-0 text-right">
                <div
                  ...
                  style={{
                    color: Number(c.diff) === 0 ? LEGACY_COLORS.muted2 : Number(c.diff) > 0 ? LEGACY_COLORS.green : LEGACY_COLORS.red,
                  }}
                >
                  {Number(c.diff) >= 0 ? "+" : ""}{formatNumber(c.diff)}
                </div>
