...
export function DeptIOTab({
  ...
  function applyInventoryUpdate(itemId: string, quantity: number, location: string | null) {
    setItems((current) =>
      current.map((item) =>
        item.item_id === itemId
          ? {
              ...item,
              quantity,
              available_quantity: quantity - Number(item.pending_quantity ?? 0),
              location,
            }
          : item,
      ),
  ...
  async function submit() {
    ...
    try {
      ...
      if (usePackage && selectedPackage) {
        ...
        const refreshedItems = await api.getItems({ limit: 2000 });
        ...
        if (mode === "in") {
          const updated = await api.receiveInventory(payload);
          applyInventoryUpdate(updated.item_id, Number(updated.quantity), updated.location);
        } else {
          const updated = await api.shipInventory(payload);
          applyInventoryUpdate(updated.item_id, Number(updated.quantity), updated.location);
        }
      }
    ...
    <div className="pb-4">
        ...
        {DEPARTMENTS.map((entry) => {
          ...
          return (
            <button
              ...
              className="shrink-0 rounded-[14px] border px-2 py-[10px]"
              style={{
      ...
      <div className="mb-3 grid grid-cols-2 gap-2">
        <button
          ...
          className="rounded-[14px] border px-3 py-3 text-sm font-bold"
          style={{
