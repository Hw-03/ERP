...
- [[ERP/backend/app/repositories/📁_repositories]]는 DB 접근 계층(DAL)입니다. 재고·품목 조회/저장을 한 곳으로 모아 서비스가 직접 쿼리를 짜지 않게 합니다.
- [[ERP/backend/app/dependencies/📁_dependencies]]는 API 진입 시 거치는 권한·인증 의존성(관리자 PIN, 창고 관리자 확인)입니다.
- [[ERP/backend/bootstrap/📁_bootstrap]]는 DB 초기화·마이그레이션·시드를 담당합니다 (`python bootstrap_db.py --all`이 진입점).
- `routers/`는 화면이 호출하는 API 문입니다.
- `frontend/app/mes/`는 현재 실제 운영 화면입니다 (2026-06-05 이전 이름: `legacy/`).
- `_attic/`은 보관소입니다. 현재 기준으로 바로 쓰지 않습니다.

## 최근 변경 (2026-06)

> [!example]- 2026-06-08 이후 들어온 주요 변화 — 펼쳐서 보기
> - **창고 박스 자동 차감**: 박스에서 품목을 출고할 때 해당 박스의 수량이 자동으로 줄어듭니다. 차감 로직은 `backend/app/services/inv_transfer.py`의 `_deplete_boxes_if_tracking()`, 규칙 설계는 `_attic/docs/warehouse-box-depletion-design.md`. 활성화 플래그(`box-tracking`)가 OFF일 때는 기존 방식으로 동작합니다.
> - **창고 지도 통합 탭 (2탭)**: 이전에는 3탭(박스 배치/재고 관리/앵글 편집)이었으나 2026-06-23에 2탭(박스 관리·앵글 편집)으로 통합됐습니다. `DesktopWarehouseMapView.tsx`가 반영된 최신 뷰입니다.
> - **인수인계 권한 = 받는 부서만**: 고압/진공 소속 직원만 인수 대기함을 보고 인수 확인을 할 수 있습니다. 생산부 결재권자라도 소속 부서가 고압/진공이 아니면 인수 불가. 의도된 설계 (2026-06-16, `backend/app/routers/handover.py`).
> - **생산 가능수량 3지표**: `ship_ready`(출하 대기)·`fast_production`(빠른 생산)·`total_production`(총생산). AF 기준으로 재정의됐으며 `services/production_capacity.py`의 `compute_af_capacity`가 기준. 구 지표(`immediate`/`maximum`)는 Phase3 전환 완료 전까지 병행 유지.
