...
> [!info] 이 문서는
> 인수인계 첫 주에 가장 자주 나오는 질문들을 모았다.
> 여기서 못 찾으면 [[위험지대_지도]] → [[처음_읽는_사람]] 순서로 이어서 읽는다.
...
그리고 `inventory/` 는 단일 파일이 아닌 **패키지(폴더)** 로 분리됐다 — `inventory/__init__.py` + `inventory/transactions.py` 등. "inventory.py 가 없어요" 라는 혼동이 자주 있다.
...
> [!note]
> 모르는 게 있으면 혼자 추측해서 코드를 바꾸지 말고, **먼저 물어본다.**
