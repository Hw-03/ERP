...
경력 1년 2개월 신입이 무심코 한 줄 고치면, 재고 수량·결재·시간이 어긋난다.

이 문서는 "어디가 위험한지" 와 "왜 위험한지" 를 알려준다.
...
## 👶 신입이 첫 주에 만지기 쉬운 영역 → 어느 위험지대인가

신입에게 자주 떨어지는 작업과, 그 작업이 건드릴 가능성이 높은 위험지대 매핑이다.
...
git log 에서 위험지대 관련 commit 만 골라 모은다. 신입이 "이 분야 어떻게 발전해 왔지?" 알고 싶을 때 참고.
...
- 2026-05-21: 최초 작성. 7개 위험지대 + "묻고 손대라" 폴더 + 검증 루프 + 신입 작업 매핑 + 3원칙.

> [!tip] 이 문서 업데이트 정책
> 위험지대가 새로 발견되면(또는 기존 위험지대가 해소되면), 이 문서에 한 단락 추가하고 commit 메시지에 이유를 적는다.
> "왜 이 영역이 위험한가" 를 사후에 못 쓰면, 그 위험은 다음 신입에게 똑같이 떨어진다.
