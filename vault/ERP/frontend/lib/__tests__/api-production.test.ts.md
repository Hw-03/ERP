...
source_path: "frontend/lib/__tests__/api-production.test.ts"
importance: normal
layer: frontend
graph: file
updated: 2026-05-22
...
# api-production.test.ts — api-production.test.ts 설명

## 이 파일은 무엇을 책임지나

`api-production.test.ts`는 TypeScript/React 코드입니다. 프로젝트 구조 안에서 `frontend/lib/__tests__/api-production.test.ts` 위치에 있으며, 필요할 때 역할과 연결 파일을 확인하기 위한 설명을 둡니다.
