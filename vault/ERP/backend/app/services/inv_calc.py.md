...
- `_sync_total()` — `InventoryLocation` 변경 후 `Inventory.quantity` 동기화
...
`_sync_total`을 누락하면 `Inventory.quantity`가 실제와 달라져 KPI 화면에 잘못된 수치가 표시됩니다.

## 연결되는 파일

### 먼저 볼 파일
- [[ERP/backend/app/services/inv_base.py]] — 의존성
- [[ERP/backend/app/services/inv_transfer.py]] — _sync_total 호출자
- [[ERP/backend/app/services/stock_math.py]] — 사용 가능 수량 계산

> [!info]- 더 연결된 파일
> - [[ERP/backend/app/services/inv_defective.py]]
> - [[ERP/backend/app/models/inventory.py]]
