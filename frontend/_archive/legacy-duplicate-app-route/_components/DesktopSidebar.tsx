...
export function DesktopSidebar({
  ...
  return (
    <aside
      className="flex h-screen w-[278px] shrink-0 flex-col border-r px-5 py-5"
      style={{ background: LEGACY_COLORS.s1, borderColor: LEGACY_COLORS.border }}
    >
      <div
        className="desktop-shell-subpanel mb-5 overflow-hidden px-4 py-4"
        style={{
          background: `linear-gradient(180deg, ${LEGACY_COLORS.s2} 0%, ${LEGACY_COLORS.s1} 100%)`,
          borderColor: LEGACY_COLORS.borderStrong,
        }}
      >
        <div className="mb-4 flex items-center justify-between">
          <div className="overflow-hidden rounded-xl bg-white px-3 py-2 shadow-sm">
          ...
          <div
            ...
            style={{ borderColor: LEGACY_COLORS.borderStrong, color: LEGACY_COLORS.blue, background: LEGACY_COLORS.blueSoft }}
          >
            ERP
          </div>
        </div>
        <div className="text-sm font-semibold leading-6">생산, 자재, 출하 흐름을 한 화면 흐름으로 빠르게 제어하는 데스크톱 워크스페이스</div>
      </div>
        ...
        {TABS.map((tab) => {
          ...
          return (
            <button
              ...
              className="group relative flex w-full items-center gap-3 overflow-hidden rounded-[24px] border px-4 py-4 text-left transition"
              style={{
                background: active
                  ? `linear-gradient(135deg, color-mix(in srgb, ${tab.accent} 22%, ${LEGACY_COLORS.s2}) 0%, ${LEGACY_COLORS.s2} 100%)`
                  : LEGACY_COLORS.s1,
            ...
            >
              <span
                className="absolute inset-y-3 left-0 w-[4px] rounded-full transition"
              ...
              <ArrowUpRight
                ...
                style={{ color: active ? tab.accent : "transparent" }}
              />
            </button>
          );
        })}
      ...
      <div className="mt-auto space-y-3">
        <div className="desktop-shell-subpanel px-4 py-4">
          ...
          <div className="text-sm leading-6" style={{ color: LEGACY_COLORS.textSoft }}>
