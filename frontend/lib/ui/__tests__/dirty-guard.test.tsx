...
describe("dirty-guard", () => {
  ...
  it('"저장하고 이동" — save 호출 후 proceed 호출', async () => {
    const proceed = vi.fn();
  ...
  // 케이스 5 — "저장하지 않고 이동" → save 호출 안 함, proceed 호출
  it('"저장하지 않고 이동" — save 미호출, proceed 호출', () => {
    const proceed = vi.fn();
    const save = vi.fn();
    render(
      <DirtyGuardProvider>
  ...
  // 케이스 8 — useLocalDirtyGuard 단독 사용 (dirty=true → 모달, 저장하지 않고 이동)
  it("useLocalDirtyGuard — dirty=true → 모달, 저장하지 않고 이동 동작", () => {
    const proceed = vi.fn();
    const save = vi.fn();
    render(
      <DirtyGuardProvider>
