...
const WORK_PANEL_HEIGHT = 810;
const LEFT_PANEL_SCALE = 0.82;
const RIGHT_PANEL_SCALE = 0.8;
...
export function DesktopWarehouseView({
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(820px,1.45fr)_minmax(420px,0.88fr)] items-start gap-4 px-6 py-6">
      <section
        className="flex min-h-0 flex-col overflow-hidden rounded-[28px] border"
        style={{ background: LEGACY_COLORS.s1, borderColor: LEGACY_COLORS.border, height: `${WORK_PANEL_HEIGHT}px` }}
      >
        <div className="border-b px-5 py-4" style={{ borderColor: LEGACY_COLORS.border }}>
          <div className="text-[32px] font-black leading-none">입출고 처리</div>
        </div>

        <div className="min-h-0 flex-1 overflow-hidden px-5 py-5">
          <div
            className="flex h-full min-h-0 flex-col rounded-[30px] border px-5 py-5"
            style={{
              background: "linear-gradient(180deg, rgba(14,18,31,.96) 0%, rgba(10,14,24,.94) 100%)",
              borderColor: LEGACY_COLORS.border,
              transform: `scale(${LEFT_PANEL_SCALE})`,
              transformOrigin: "top left",
              width: `${100 / LEFT_PANEL_SCALE}%`,
              height: `${100 / LEFT_PANEL_SCALE}%`,
            }}
          >
            ...
            <div className="my-5 shrink-0 border-t" style={{ borderColor: LEGACY_COLORS.border }} />

            <div className="mb-3 shrink-0 text-center text-sm font-bold" style={{ color: LEGACY_COLORS.muted2 }}>
              품목 선택
            </div>
            ...
            <div className="flex min-h-0 flex-1 flex-col">
              ...
              <div className="mb-2 shrink-0 flex items-center gap-3 rounded-[16px] border px-4 py-3" style={{ background: LEGACY_COLORS.s2, borderColor: LEGACY_COLORS.border }}>
              ...
              <div className="mb-2 shrink-0 text-[11px]" style={{ color: LEGACY_COLORS.muted2 }}>
                {workType === "package-out" ? `${formatNumber(filteredPackages.length)}개 패키지` : `${formatNumber(filteredItems.length)}개 품목`}
              </div>

              {workType === "package-out" ? (
                <div className="min-h-0 flex-1 overflow-y-auto rounded-[18px] border" style={{ background: LEGACY_COLORS.s2, borderColor: LEGACY_COLORS.border }}>
      ...
      >
        <div className="border-b px-5 py-4" style={{ borderColor: LEGACY_COLORS.border }}>
          <div className="text-[28px] font-black leading-none">실행 확인</div>
        </div>

        <div
          className="flex min-h-0 flex-1 flex-col overflow-hidden"
          style={{
            transform: `scale(${RIGHT_PANEL_SCALE})`,
            transformOrigin: "bottom left",
            width: `${100 / RIGHT_PANEL_SCALE}%`,
            height: `${100 / RIGHT_PANEL_SCALE}%`,
          }}
        >
          {!selectedItem && !selectedPackage ? (
            <>
              ...
              <div
                className="mt-auto flex min-h-[108px] shrink-0 items-center border-t px-5 py-4"
                style={{ borderColor: LEGACY_COLORS.border, background: LEGACY_COLORS.s1 }}
              >
                <button disabled className="flex w-full items-center justify-center gap-2 rounded-2xl px-4 py-3 text-base font-bold text-white opacity-40" style={{ background: LEGACY_COLORS.blue }}>
            ...
            <>
              <div className="min-h-0 flex-1 overflow-y-auto px-5 py-5">
                <div className="flex min-h-full flex-col rounded-[24px] border p-4" style={{ background: LEGACY_COLORS.s2, borderColor: LEGACY_COLORS.border }}>
                  ...
                  />
                    ...
                    <textarea
                      ...
                      placeholder="비고"
                      className="min-h-[92px] flex-1 w-full rounded-[14px] border px-4 py-3 text-sm outline-none"
                      style={{ background: LEGACY_COLORS.s1, borderColor: LEGACY_COLORS.border, color: LEGACY_COLORS.text }}
                    />
                  </div>
                </div>
