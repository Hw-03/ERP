...
function SummaryCard({
  ...
  return (
    <button
      onClick={onClick}
      className="relative min-h-[78px] flex-1 overflow-hidden rounded-xl border px-3 py-2 text-left transition hover:opacity-95"
      style={{
    ...
    >
      ...
      <div className="font-mono text-[24px] font-black leading-none" style={{ color }}>
        {formatNumber(value)}
      </div>
...
function InsightCard({
  ...
  return (
    <button
      onClick={onClick}
      className="min-h-[62px] flex-1 rounded-xl border px-3 py-2 text-left transition hover:opacity-95"
      style={{ borderColor: LEGACY_COLORS.border, background: LEGACY_COLORS.s2 }}
    >
      <div className="mb-2 flex items-center gap-2 text-[11px] font-bold" style={{ color: LEGACY_COLORS.muted2 }}>
        {icon}
        {title}
      </div>
      <div className="text-sm font-bold">모델별 생산 가능 수량 보기</div>
      <div className="mt-1 text-[11px] leading-4" style={{ color: LEGACY_COLORS.muted2 }}>
        {description}
      </div>
    </button>
  );
...
export function DesktopInventoryView({
  ...
  return (
    <div className="flex min-h-0 flex-1 gap-4" style={{ padding: "14px" }}>
      <div className="flex min-h-0 min-w-0 flex-1 flex-col overflow-hidden rounded-[36px] border" style={{ background: LEGACY_COLORS.s1, borderColor: LEGACY_COLORS.border }}>
        <div className="border-b px-6 pb-5 pt-6" style={{ borderColor: LEGACY_COLORS.border, background: "rgba(8,10,16,.74)" }}>
        ...
        {error ? (
          <div className="flex-1 p-8 text-sm" style={{ color: LEGACY_COLORS.red }}>
            {error}
          </div>
        ) : loading ? (
          <div className="flex-1 p-8 text-sm" style={{ color: LEGACY_COLORS.muted2 }}>
            재고 데이터를 불러오는 중입니다...
          </div>
        ) : (
          <div className="p-4">
            <div
              className="overflow-y-auto rounded-[28px] border"
              style={{ height: "calc(100vh - 410px)", borderColor: LEGACY_COLORS.border, background: LEGACY_COLORS.s1 }}
            >
      ...
      >
        {!selectedItem ? (
          <div className="flex min-h-[320px] h-full flex-col items-center justify-center gap-3 rounded-[28px] border p-4 text-center" style={{ background: LEGACY_COLORS.s2, borderColor: LEGACY_COLORS.border }}>
            <div className="rounded-[20px] p-3" style={{ background: "rgba(79,142,247,.12)", color: LEGACY_COLORS.blue }}>
              <PackageSearch size={22} />
            </div>
            <div className="text-sm font-bold">품목을 선택해 주세요</div>
            <div className="max-w-[200px] text-[12px] leading-5" style={{ color: LEGACY_COLORS.muted2 }}>
