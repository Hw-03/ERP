...
export default function CategoryCard({
  ...
  return (
    <div
      className={`${cardClassName} transition-all duration-200 hover:border-opacity-100 hover:shadow-xl hover:-translate-y-0.5`}
    >
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span
                className={`badge font-mono font-bold text-sm ${meta.badge} px-2 py-0.5 rounded`}
              >
                {data.category}
              </span>
              {isAlert && (
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="bg-slate-900/50 rounded-lg p-3">
          ...
          <p className="text-slate-500 text-xs">EA 기준</p>
        </div>
      </div>
      ...
      {data.item_count > 0 && totalQty === 0 && (
        <div className="mt-3 flex items-center gap-1.5 text-amber-400 text-xs">
          <span>주의</span>
          <span>현재 재고가 0입니다. 입고 또는 확인이 필요합니다.</span>
        </div>
      )}
    </div>
  );
