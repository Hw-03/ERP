    ...
    <div className="mb-3 flex gap-[7px] overflow-x-auto">
      {SECTIONS.map((entry) => (
        <button
          ...
          className="shrink-0 rounded-full border px-4 py-[7px] text-xs font-bold"
          style={{
      ...
      <button
        onClick={onLock}
        className="ml-auto shrink-0 rounded-full border px-4 py-[7px] text-xs font-bold"
    ...
    <>
      ...
      <div className="overflow-hidden rounded-[14px] border" style={{ background: LEGACY_COLORS.s1, borderColor: LEGACY_COLORS.border }}>
        {visibleItems.map((item, index) => (
          <button key={item.item_id} onClick={() => open(item)} className="flex w-full items-center justify-between px-[14px] py-3 text-left" style={{ borderBottom: index === visibleItems.length - 1 ? "none" : `1px solid ${LEGACY_COLORS.border}` }}>
            <div>
      ...
      <BottomSheet open={!!selected} onClose={() => setSelected(null)} title={selected?.item_name || "상품 편집"}>
        <div className="space-y-3 px-5 pb-6">
          ...
          <button onClick={() => void save()} className="w-full rounded-xl py-[13px] text-[15px] font-bold text-white" style={{ background: LEGACY_COLORS.blue }}>
            저장
          </button>
        </div>
      </BottomSheet>
