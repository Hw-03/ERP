  ...
  return (
    <div className="hidden min-h-screen bg-black lg:block">
      ...
      >
        ...
        {historyOpen ? (
          <div className="absolute inset-y-0 right-0 z-30 w-[420px] border-l px-5 py-5 shadow-2xl" style={{ background: LEGACY_COLORS.s1, borderColor: LEGACY_COLORS.border }}>
            <div className="mb-4 flex items-center justify-between">
              <div>
                <div className="text-lg font-black">입출고 내역</div>
                <div className="mt-1 text-xs" style={{ color: LEGACY_COLORS.muted2 }}>
                  최근 40건 거래 이력을 빠르게 확인합니다.
                </div>
              </div>
              <button onClick={() => setHistoryOpen(false)} className="rounded-2xl border p-2" style={{ borderColor: LEGACY_COLORS.border }}>
                <X className="h-4 w-4" />
              </button>
            </div>
            ...
            <div className="space-y-2 overflow-y-auto pb-6" style={{ height: "calc(100vh - 180px)" }}>
              {history.map((log) => (
                <div key={log.log_id} className="rounded-3xl border p-4" style={{ background: LEGACY_COLORS.s2, borderColor: LEGACY_COLORS.border }}>
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <span className="rounded-full px-2.5 py-1 text-[11px] font-bold" style={{ background: "rgba(79,142,247,.14)", color: transactionColor(log.transaction_type) }}>
