...
import {
  ...
  formatNumber,
  processStageLabel,
} from "@/legacy/_components/legacyUi";
...
export default function QueuePage() {
  ...
  const doToggleLine = async (batchId: string, lineId: string, included: boolean) => {
    try {
      ...
      setBatches((prev) =>
        prev.map((b) => (b.batch_id === updated.batch_id ? updated : b)),
      );
    } catch (e) {
              ...
              <div className="flex gap-2">
                {(["PRODUCE", "DISASSEMBLE", "RETURN"] as const).map((t) => (
                  <button
                    ...
                    style={{
                      background:
                        batchType === t ? BATCH_TYPE_COLOR[t] : LEGACY_COLORS.s3,
            ...
            <div className="px-4 pt-3">
                ...
                >
                  <div>
                    ...
                    <div className="text-[11px]" style={{ color: LEGACY_COLORS.muted2 }}>
                      ...
                      {formatNumber(
                        selectedBatchItem.available_quantity ?? selectedBatchItem.quantity,
                      )}{" "}
                ...
                >
                  {filteredBatchItems.map((item, i) => (
                    <button
                      key={item.item_id}
                      onClick={() => {
                        setBatchItemId(item.item_id);
                        setBatchSearch(item.item_name);
                      }}
                      className="flex w-full items-center justify-between px-3 py-2 text-left"
                      style={{
                        borderBottom:
                          i < filteredBatchItems.length - 1
              ...
              >
                <input
                  type="checkbox"
                  checked={loadBom}
                  onChange={(e) => setLoadBom(e.target.checked)}
                />
          ...
          <div className="space-y-3">
            {batches.map((b) => {
              const varianceLines = b.lines.filter(
                (ln) =>
                  ln.bom_expected != null &&
                  Number(ln.bom_expected) !== Number(ln.quantity),
              );
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="mb-1 flex flex-wrap items-center gap-[6px]">
                        <span
                          ...
                          style={{
                            background: `${BATCH_TYPE_COLOR[b.batch_type]}22`,
                            color: BATCH_TYPE_COLOR[b.batch_type],
                          }}
                        >
                      ...
                      <div className="truncate text-sm font-semibold">
                        {b.parent_item_name ?? "-"}
                        {b.parent_quantity != null
                          ? ` × ${formatNumber(b.parent_quantity)}`
                          : ""}
                      </div>
                      <div
                      ...
                      >
                        ...
                        {b.confirmed_at
                          ? ` · 확정 ${new Date(b.confirmed_at).toLocaleString("ko-KR")}`
                          : ""}
                      </div>
                    </div>
                    {b.status === "OPEN" && (
                      ...
                      return (
                        ...
                        >
                          ...
                          {b.status === "OPEN" && (
                            <button
                              onClick={() =>
                                void doToggleLine(b.batch_id, ln.line_id, !ln.included)
                              }
                              className="shrink-0 text-[14px]"
                              title={ln.included ? "제외하기" : "포함하기"}
                              style={{
                                color: ln.included ? LEGACY_COLORS.green : LEGACY_COLORS.muted,
                              }}
                            >
                              {ln.included ? "✓" : "—"}
                            </button>
                          ...
                          <span
                            ...
                            style={{
                              background: DIR_BG[ln.direction],
                              color: DIR_COLOR[ln.direction],
                            }}
                          >
                                  ...
                                  onKeyDown={(e) => {
                                    if (e.key === "Enter")
                                      void doOverrideLine(
                                        b.batch_id,
                                        ln.line_id,
                                        lineQtyEdit[ln.line_id],
                                      );
                                    if (e.key === "Escape")
                                      setLineQtyEdit((prev) => {
                                        const next = { ...prev };
                                        delete next[ln.line_id];
                                        return next;
                                      });
                                  }}
                                />
                        ...
                        return (
                          ...
                          >
                            ...
                            <span style={{ color: LEGACY_COLORS.muted }}>
                              예상 {formatNumber(ln.bom_expected)} → 실제{" "}
                              {formatNumber(ln.quantity)}
                            </span>
                            <span
                              style={{
                                color: diff > 0 ? LEGACY_COLORS.red : LEGACY_COLORS.green,
                              }}
                            >
                              ({diff > 0 ? "+" : ""}
                              {formatNumber(diff)})
