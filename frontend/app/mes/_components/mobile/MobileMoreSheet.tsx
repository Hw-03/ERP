...
import { BarChart2, MapPinned } from "lucide-react";
import { BottomSheet } from "@/lib/ui/BottomSheet";
import { SheetHeader, MoreMenuRow } from "./primitives";

/**
 * MobileMoreSheet — 하단 '더보기' 버튼이 여는 기능 네비 시트.
 * 하단 탭바에서 강등된 화면(주간보고·창고지도)으로의 진입점.
 * (관리 화면은 모바일에서 제외 — 관리 작업은 데스크톱에서.)
 * 프로필/PIN/로그아웃을 다루는 MobileUserMenuSheet 와는 의도적으로 분리(통합 금지).
 *
 * MobileTabId 에 직접 의존하지 않도록 진입 동작을 콜백으로 받는다.
 * `onWarehouseMap` 미전달 시 창고지도 행은 "준비 중" 비활성.
 */
