...
describe("DesktopWeeklyReportView F705-02 다운로드", () => {
  ...
  it("keeps the full production card and download control while loading", () => {
    state.useWeeklyReportQuery.mockReturnValue({ data: undefined, isLoading: true, error: null });
    renderWeekly();
    expect(screen.getByTestId("weekly-production-card")).toHaveClass("weekly-production", "weekly-production-placeholder");
  ...
  it("생산 기록 유무와 관계없이 같은 카드 높이와 다운로드 위치를 유지한다", () => {
    const { rerender } = renderWeekly();
    const populatedCard = screen.getByTestId("weekly-production-card");
    const populatedHeight = populatedCard.style.minHeight;
    ...
    expect(emptyCard).toHaveClass("weekly-card", "weekly-production", "weekly-production-placeholder");
    expect(emptyAnchor).toHaveClass("weekly-download");
    expect(emptyCard.style.minHeight).toBe(populatedHeight);
