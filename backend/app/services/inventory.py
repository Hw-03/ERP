...
def consume_pending(db: Session, item_id: uuid.UUID, qty: Decimal) -> Inventory:
    ...
    if figure.physical - figure.active_shipping_reserved < qty:
        raise ValueError(
            "창고 가용 재고 부족 "
            f"(물리 {figure.physical}, 출하예약 {figure.active_shipping_reserved}, "
            f"차감 요청 {qty})."
        )
...
def adjust_warehouse(
    ...
    *,
    ...
    if delta < 0:
        ...
        if new_warehouse_qty < reserved_floor:
            raise ValueError(
                "창고 재고를 재고 요청·출하 예약 수량보다 낮게 조정할 수 없습니다."
            )
