...
def _shipping_reservations(
    ...
    for (item_id, department), quantity in sorted(grouped.items()):
        ...
        if stock + delta < pending + restored:
            blockers.append("출하 예약을 복원할 재고가 부족합니다.")
        snapshot.append({"item_id": str(item_id), "department": department, "quantity": quantity,
                         "stock": stock, "combined_reserved": pending, "reserved": reserved})
    return snapshot


def _reservation_increases(
    db: Session,
    effects: list[InventoryOperationEffect],
    cell_keys,
) -> dict[tuple[str, str, str, str, str], int]:
    """취소 후 RESERVED로 복원되는 출하 배정을 물리 셀별로 합산한다."""

    keys = set(cell_keys)
    increases: dict[tuple[str, str, str, str, str], int] = {}
    for effect in effects:
        if effect.subject_type != "ShippingAllocation":
            continue
        if _effect_subject_plan(db, effect).target_state.get("status") != "RESERVED":
            continue
        allocation = db.get(ShippingAllocation, effect.subject_id)
        if allocation is None:
            raise CancellationNotAllowed("연결된 출하 배정을 찾을 수 없습니다.")
        location_key = (
            ...
            "location",
            str(allocation.department or ""),
            LocationStatusEnum.PRODUCTION.value,
            "",
        )
        warehouse_key = (str(allocation.item_id), "warehouse", "", "", "")
        key = warehouse_key if allocation.department is None else location_key
        if key in keys:
...
def preview_cancellation(
    ...
    *,
    ...
    try:
        reservation_increases = _reservation_increases(
            db,
            operation_effects,
            changes,
        )
    ...
    for key, quantity_change in sorted(changes.items()):
        ...
        if after < reserved + reservation_increases.get(key, 0):
    ...
    for record_id, quantity_change in sorted(movement_changes.items()):
        ...
        reserved = int(defect_records_svc.pending_quantity(db, record.record_id))
        after = current + quantity_change
...
def _reverse_log(
    ...
    *,
    ...
    before_cells = inv_effect.snapshot_cells(db, original.item_id)
    ...
    if effect:
        inv_effect.apply_effect_reverse(db, original.item_id, effect)
...
def _reverse_defect_movement(
    ...
    *,
    ...
    pending = int(defect_records_svc.pending_quantity(db, record.record_id))
...
def _assert_plan_applied(
    ...
    *,
    ...
    for cell in plan.cells:
        ...
        expected_reserved = cell.reserved_quantity + reservation_increases.get(key, 0)
...
def cancel_operation(
    ...
    *,
    ...
    with transactional(db):
        ...
        affected_item_ids = sorted({log.item_id for log in logs} | allocation_items)
        inventory_svc.lock_inventories(db, affected_item_ids)
        if affected_item_ids:
            location_query = (
                db.query(InventoryLocation)
                .filter(InventoryLocation.item_id.in_(affected_item_ids))
                .order_by(
        ...
        cancellation = operation_svc.create_cancellation_operation(
            db,
            original=original,
            actor_name=canceller.name,
            actor_employee_id=canceller.employee_id,
