...
def capture_log_stock_snapshot(
    ...
    return {
        ...
        "inventory_effect": capture_effect(db, item_id, before, after),
    }
...
def capture_effect(
    ...
    return effect_diff(before, after if after is not None else snapshot_cells(db, item_id))
...
def apply_effect_reverse(db: Session, item_id: uuid.UUID, effect: list[dict] | None) -> None:
    ...
    for cell in effect or []:
        ...
        if cell.get("scope") == "warehouse":
            ...
            reserved_floor = pending + int(figure.active_shipping_reserved)
            if new_val < reserved_floor:
                raise ValueError(
                    f"취소 후 창고 재고({new_val})가 재고 요청·출하 예약 수량"
                    f"({reserved_floor})보다 작아집니다."
                )
            ...
            if status == LocationStatusEnum.PRODUCTION:
                ...
                shipping_reserved = int(figure.active_shipping_reserved)
            reserved_floor = pending + shipping_reserved
            if new_val < reserved_floor:
                raise ValueError(
                    f"취소 후 {dept} {status.value} 재고({new_val})가 "
                    f"재고 요청·출하 예약 수량({reserved_floor})보다 작아집니다."
                )
