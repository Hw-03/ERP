...
@pytest.mark.parametrize(
    ("submitted_work_type", "expected_status"),
    [("receive", 403), ("ship", 422)],
)
def test_io_submit_receive_rejects_non_warehouse_requester_without_mutation(
    client, db_session, make_item, submitted_work_type, expected_status
):
    item = make_item(name="Unauthorized Raw", warehouse_qty=Decimal("0"))
    requester = _make_employee(db_session, code="IO-RECEIVE-NONE")
    db_session.commit()
    preview = client.post(
        "/api/io/preview",
        json={
