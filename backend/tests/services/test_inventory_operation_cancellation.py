...
def test_cancel_blocks_when_reversal_would_intrude_on_shipping_reservation(
    db_session, make_item
) -> None:
    """입고 취소가 활성 출하 예약의 물리 재고를 없애면 차단한다."""

    item = make_item(name="출하 예약 침범 취소", warehouse_qty=Decimal("0"))
    actor = _actor(db_session)
    db_session.add(
        SystemSetting(
            setting_key="inventory_operation_cutover_at",
