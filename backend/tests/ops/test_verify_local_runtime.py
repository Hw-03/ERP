    ...
    *,
    ...
    (fake_bin / "npm.cmd").write_text(
        "\r\n".join(
            [
                "@echo off",
                '>>"%DEXCOWIN_MOCK_GATE_LOG%.frontend-tsc-incremental" echo frontend-tsc-incremental start %*',
                "ping -n 4 127.0.0.1 >nul",
                '>>"%DEXCOWIN_MOCK_GATE_LOG%.frontend-tsc-incremental" echo frontend-tsc-incremental end %*',
                "exit /b 0",
                "",
            ]
        ),
...
def test_smart_targeted_gates_run_in_parallel_and_merge_each_timing(tmp_path: Path) -> None:
    ...
    typecheck_args = _gate_event_lines(gate_log, "frontend-tsc-incremental")[0]
    assert "--max-warnings=0" in lint_args
    assert "app/sample.ts" in lint_args
    assert "app/sample.test.ts" in lint_args
    assert "run typecheck:app -- --incremental" in typecheck_args
