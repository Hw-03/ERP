...
function Stat({ label, value, unit, tone, small }) {
  const color = tone === "out" ? "#E5576E" : tone === "low" ? "#E0A63C" : "#E6ECF5";
  return (
    <div className="bg-[#0E1A34] px-4 py-3">
      <div className="text-[10px] text-[#8A97B3]">{label}</div>
      <div className="mt-0.5 flex items-baseline gap-1">
        <span className={small ? "text-[16px] font-semibold" : "text-[22px] font-semibold tracking-tight"}
              style={{ color, fontVariantNumeric:"tabular-nums" }}>{value}</span>
...
function HistRow({ ts, type, qty, who }) {
  const color = qty.startsWith("+") ? "#4FB37A" : qty.startsWith("-") ? "#E5576E" : "#E6ECF5";
  return (
    <li className="flex items-center gap-2">
      <span className="text-[10px] text-[#6C7A99] font-mono w-20">{ts}</span>
      <span className="text-[11px] text-[#B6C1DC] w-8">{type}</span>
      <span className="font-semibold flex-1" style={{color, fontVariantNumeric:"tabular-nums"}}>{qty}</span>
      <span className="text-[11px] text-[#8A97B3]">{who}</span>
    </li>
  );
}
