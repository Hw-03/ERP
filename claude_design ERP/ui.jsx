...
/* ---------- Search input ---------- */
function SearchInput({ value, onChange, placeholder, right, size = "md" }) {
  const h = size === "sm" ? 28 : size === "lg" ? 40 : 32;
  return (
    <div className="flex items-center gap-2 rounded-md bg-[#132240] border border-[#263858] focus-within:border-[#2B5C58] px-2.5"
         style={{ height: h }}>
      <Ic.Search className="w-3.5 h-3.5 text-[#6C7A99] shrink-0"/>
      <input value={value ?? ""} onChange={(e) => onChange?.(e.target.value)} placeholder={placeholder}
        className="flex-1 bg-transparent outline-none text-[12px] text-[#E6ECF5] placeholder:text-[#5A6785]"/>
      {right}
    </div>
  );
}

Object.assign(window, { TOKENS, Ic, StatusDot, Chip, Btn, AppWindow, Sidebar, TopBar, KPI, Section, SearchInput });
